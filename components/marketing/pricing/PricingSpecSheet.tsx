...
const TIERS = [
  { id: "developer", name: "Developer", price: "Free", sends: "1k", overage: "N/A" },
  { id: "growth", name: "Growth", price: "$99", sends: "50k", overage: "$0.002" },
  { id: "scale", name: "Scale", price: "$299", sends: "250k", overage: "$0.0015" },
  { id: "enterprise", name: "Enterprise", price: "Custom", sends: "1M+", overage: "Custom" },
];

const FEATURES = [
  { label: "Channels", developer: "Email", growth: "All", scale: "All", enterprise: "All" },
  { label: "ZK Receipts", developer: true, growth: true, scale: true, enterprise: true },
  { label: "API Keys", developer: "1", growth: "5", scale: "∞", enterprise: "∞" },
  { label: "Team Space", developer: "1", growth: "3", scale: "10", enterprise: "∞" },
  { label: "Uptime SLA", developer: false, growth: "99.9%", scale: "99.9%", enterprise: "99.99%" },
  { label: "Support", developer: "Base", growth: "Standard", scale: "Priority", enterprise: "24/7 CSM" },
  { label: "Custom DKIM", developer: false, growth: false, scale: true, enterprise: true },
  { label: "Nitro TEE", developer: true, growth: true, scale: true, enterprise: true },
];
      ...
      <div className="container mx-auto px-6 max-w-5xl">
        ...
        <div className="border border-border-hi bg-bg-surface overflow-hidden rounded-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1.5fr_1fr_1fr_1fr_1fr] border-b border-border-hi">
            ...
            <div className="md:hidden p-8 flex flex-col items-center gap-3 text-center bg-teal/3">
                {TIERS.filter(t => t.id === activeTier).map(tier => (
                    <div key={tier.id} className="flex flex-col gap-2 animate-in fade-in slide-in-from-top-2 duration-500">
                        <span className="text-[10px] font-bold font-mono tracking-[0.2em] text-teal uppercase">{tier.name}</span>
                        <span className="text-4xl font-black text-text-primary px-0 leading-none">{tier.price}</span>
                        <span className="text-[10px] font-mono text-text-muted uppercase tracking-widest">{tier.sends} sends included</span>
                    </div>
                ))}
            </div>
            ...
            {TIERS.map((tier) => (
              ...
              >
                <span className="text-[10px] font-bold font-mono tracking-widest text-teal uppercase">{tier.name}</span>
                <span className="text-2xl font-black text-text-primary">{tier.price}</span>
                <span className="text-[10px] font-mono text-text-muted uppercase">{tier.sends} sends</span>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1.5fr_1fr_1fr_1fr_1fr] border-t border-border-hi">
            ...
            {TIERS.map((tier) => (
              ...
              >
                <button className={`w-full py-3.5 font-bold text-[10px] uppercase tracking-widest transition-all rounded-lg border ${
                    tier.id === activeTier ? "bg-teal text-bg-base border-teal shadow-lg shadow-teal/10" : "border-border-hi hover:border-teal/50 hover:text-teal"
                }`}>
        ...
        <div className="mt-12 flex flex-col md:flex-row justify-between items-center gap-6 text-text-muted opacity-80">
            <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono uppercase tracking-[0.2em] cursor-default">Scalable Protocol Infrastructure</span>
            </div>
...
function FeatureValue({ value }: { value: string | boolean }) {
  ...
  return <span className="text-xs font-mono text-text-secondary font-bold tracking-tight">{value}</span>;
}
